...
(reg-sub :get-discoveries
  (fn [db]
    (:discoveries db)))

(reg-sub :get-current-tag
  (fn [db]
    (:current-tag db)))

(reg-sub :get-discover-search-tags
  (fn [db]
    (:discover-search-tags db)))
...
;; TODO janherich: is this really what we want, what about `state-of-dapps` group ?
